...
io_test() {
    (LANG=C dd if=/dev/zero of=test_$$ bs=64k count=32k conv=fdatasync && rm -f test_$$ ) 2>&1 | awk -F, '{io=$NF} END { print io}' | sed 's/^[ \t]*//;s/[ \t]*$//'
...
io4=$( io_test )
echo -e "I/O speed(4th run)   : ${YELLOW}$io4${PLAIN}"
io5=$( io_test )
echo -e "I/O speed(5th run)   : ${YELLOW}$io5${PLAIN}"
...
[ "`echo $io5 | awk 'NR==1 {print $2}'`" == "GB/s" ] && ioraw5=$( awk 'BEGIN{print '$ioraw5' * 1024}' )
